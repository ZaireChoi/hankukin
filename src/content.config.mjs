...
const baseFields = (image) => ({
  ...
  /**
   * 번역본의 대표사진 대체텍스트 (번역본에만 적는다).
   *
   * 왜 필드가 둘인가 (2026-08-22).
   *
   *   대표사진 검사의 주석은 예전부터 이렇게 선언해 놓았다 —
   *   「사진을 frontmatter 에 복사해 두면 원문에서 사진을 바꿀 때
   *     번역본만 옛 사진을 가리키게 된다. **사진은 한 곳에만 적는다.**」
   *   그런데 코드는 hero 를 **그 문서에서만** 읽었다. 그래서 실제로는 이랬다:
   *
   *     번역본에 hero 를 안 적으면 → 일본어·중국어 쪽에 사진이 아예 안 나온다 (9쪽)
   *     적으면                    → 나오지만 원문에서 바꿀 때 번역본이 옛 사진에 남는다 (9쪽)
   *
   *   이태원 편 대표사진을 바꿀 때 파일 셋을 손으로 다 고쳤고,
   *   하나라도 빠뜨렸으면 일본 독자만 다른 사진을 봤을 것이다.
   *   어떤 게이트도 그걸 안 잡았다.
   *
   * 「번역본이 원문 hero 를 통째로 물려받는다」로 하려다 **틀렸다는 걸 알았다.**
   * 그러면 alt 까지 물려받아서 일본어 페이지에 영어 alt 가 붙는다.
   * 화면 낭독기에도, 이미지 검색에도 손해다. 그래서 가른다:
   *
   *   파일·라이선스·sourceUrl·크레딧 — **원문 한 곳.** 사실이고 언어와 무관하다
   *   alt                            — **언어마다 따로.** 이건 사실이 아니라 글이다
   *
   * 그래서 번역본은 hero 를 아예 안 적고 heroAlt 만 적는다.
   * hero 는 나머지 전부가 필수인 채로 남는다 — 원문 쪽 규율을 느슨하게 하지 않는다.
   * 둘을 헷갈려 적으면 content-quality 의 hero-translation 게이트가 세운다.
   */
