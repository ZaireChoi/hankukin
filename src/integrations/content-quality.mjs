...
import { checkKoglLicences, loadLicenceRegistry } from './lib/kogl-check.mjs';
import { LOCALES, DEFAULT_LOCALE } from '../config/brand.mjs';

const CONTENT_DIR = 'src/content';

/**
 * 공공누리 유형 대조 — 로직은 lib/kogl-check.mjs 에 있다.
 *
 * 왜 밖에 뒀나. 통합 안에 두면 **빌드를 통째로 돌려야만 시험할 수 있다.**
 * 이 게이트를 만든 날 샌드박스 디스크가 꽉 차 `npm ci` 도 못 돌렸고,
 * 시험하지 못한 게이트를 자동 푸시에 밀어 넣을 뻔했다. 자세한 사정은 그 파일 머리에 적었다.
 */
const KOGL_REGISTRY = loadLicenceRegistry();
...
const HERO_EXEMPT = {
  ...
  'kpop-album-hanteo-family-store-counts':
    '재고 332장의 슬러그를 전부 뽑아 훑었다 (2026-09-03 오후). 음반매장·앨범 매대·계산대 계열이 **0장**이다 — ' +
    '있는 것은 궁·산·시장·제주·거리 계열뿐이다. 명동 5장 중 유일한 미사용분인 myeongdong-2 는 ' +
    '이미 두 번 기록된 오매칭(명동성당: 벽돌 첨탑·잔디밭)이라 쓸 수 없고, 나머지 넷은 다른 기사가 이미 쓰고 있어 ' +
    'no-shared-images 가 막는다. 그리고 이 주제는 사진이 있어도 못 쓴다: 앨범 표지와 포토카드는 타인의 저작물이라 ' +
    '스키마의 라이선스 목록(kogl-1·kogl-3·cc0·cc-by·cc-by-sa·own)에 들어올 수 없고, 매장 계산대는 ' +
    '식별 가능한 사람이 그대로 찍히는 자리다 — 금지 항목 둘에 동시에 걸린다. ' +
    '게다가 이 기사의 주인공은 장면이 아니라 **시점**이다. 같은 앨범 한 장이 써클 앨범차트에는 ' +
    '「상자가 매장으로 나갈 때」 잡히고 한터에는 「계산할 때」 잡힌다 — 시간 축이 있어야 보이고 사진으로는 못 찍는다. ' +
    '환급편·클리닉편과 같은 판단으로 ChartAlbumCountingMoment 를 그렸다. 칸에 적힌 「무엇을 세는가」는 ' +
    '전부 그 차트 운영자가 자기 페이지에 써 놓은 문장이고, 우리가 만든 것은 시간 축 위의 배치뿐이다. ' +
    '한글은 도표에 한 자도 넣지 않았다 (샌드박스에 한글 글꼴이 없어 PNG 검수에서 네모로 나오는 문제를 원천 차단).',

  'bts-bus-stop-jumunjin-gangneung-bus-back-to-station':
    '재고의 주문진 13장(jumunjin-breakwater-1~13)을 대조표로 전부 열었다 (2026-09-05 오후). 항구 2장(1·2), ' +
    '방파제 8장(4·5·6·7·8·10 등), 모래밭 2장(3·9), tvN 도깨비 안내판 1장(13). **버스정류장 포토존은 0장**이다. ' +
    '13번 안내판은 「포스터·안내판을 사진처럼 쓰지 않는다」에 걸리고, 모래밭 2장은 주문진 어디인지 특정할 수 없어 ' +
    '「그냥 한국」 사진이 된다. 나머지는 도깨비 편이 쓰고 있거나 방파제라 이 글의 주제가 아니다. ' +
    '한국관광공사에 사진이 등재돼 있으므로(contentId 2638566) data/photo-requests.json 에 jumunjin-bts-bus-stop 으로 ' +
    '요청해 두었다 — 도착하면 이 면제를 지우고 대표사진으로 건다. ' +
    '그리고 이 기사의 요점은 정류장 모양이 아니라 **가는 버스와 오는 버스가 다르다**는 것이라, 강릉시 BIS 의 ' +
    '노선별 정차 목록을 그대로 그린 ChartBtsBusStopReturn 으로 지탱한다. 정류장 간격만 우리가 배치했고 그 사실을 도표 안에 적었다.',

  'korea-convenience-store-medicine-13-items':
    '재고 332장의 슬러그를 전부 뽑아 훑었다 (2026-09-03). 편의점·매대·약국 계열이 **0장**이다 — ' +
    '있는 것은 궁·산·시장·제주 계열뿐이고, 시장 계열(gwangjang-market·namdaemun-market)은 이미 다른 기사가 쓰고 있어 ' +
    'no-shared-images 가 막는다. 그리고 이 주제는 사진이 있어도 못 쓴다: 편의점 계산대와 매대는 ' +
    '식별 가능한 사람이 그대로 찍히는 자리이고, 그 사람은 물건을 사러 거기 있었다 — 금지 항목 그대로다. ' +
    '약 상자를 찍어 거는 것도 타인의 저작물(제품 포장 디자인)이라 스키마의 라이선스 목록에 들어올 수 없다. ' +
    '게다가 이 기사의 주인공은 장면이 아니라 **닫힌 목록**이다. 「네 구분 말고는 아무것도 없다」 — ' +
    '지사제도 알레르기약도 없다는 사실은 사진으로 찍히지 않는다. 비어 있음을 보이려면 칸을 그려야 한다. ' +
    '클리닉편·문패편과 같은 판단으로 ChartOtcShelf 를 그렸다. 구분·품목수(5·4·2·2)·합계 13 은 ' +
    '보건복지부 표를 센 것이고 표의 「계 13개 품목」과 맞는다. 우리가 더한 숫자는 없다.',

  'korean-toilet-signs-nam-yeo-door-code-paper':
    '재고 슬러그를 전부 훑었다 (2026-09-06) — 화장실·문패·개찰구 계열이 0장이다. 그리고 이 주제는 사진이 있어도 못 쓴다: ' +
    '화장실 입구와 칸 안은 식별 가능한 사람이 그대로 찍히는 자리이고, 빈 칸을 찍어도 「그냥 화장실」이라 주제(어느 규칙이 ' +
    '어디에 붙는가)를 말하지 못한다. 비데 조작판·영수증을 찍어 걸면 제조사 디자인과 업소 정보가 그대로 실린다. ' +
    '요점이 물건이 아니라 **자리 셋**(문·칸·개찰구)이라 문패편·PSD편과 같은 판단으로 ChartToiletDoorStallGate 를 그렸다. ' +
    '칸 안 배치는 시행령 제7조(휴지통 없음·위생용품 수거함·청소 중 표지)와 법 제7조④(비상벨) 서술만 그렸고, ' +
    '비데 낱말 넷은 관찰이라 도표 안에 「제조사마다 다르다」고 적었다. 15분·1,550원·1회권 제외는 서울시·국토교통부 자료 그대로다. ' +
    '우리가 직접 찍은(own) 개방화장실 표지 사진이 생기면 그때 검토하고 이 줄을 지운다.',

  'busan-gyeongsang-dialect-endings-no-na':
    '경상권 재고를 대조표로 만들어 12장을 전부 열어 봤다 (2026-09-06 오후). 미사용분은 ' +
    'geoje-windy-hill-1(바다 산책로) · geoje-windy-hill-5(거제 관광안내판) · gyeongju-cheomseongdae-2·3·4·6(첨성대) · ' +
    'gyeongju-woljeonggyo-2·4·5(월정교) · geoje-maemidong-2·4(매미성 돌담) · geoje-okpo-2(진열장 안 거북선 기념품) 이다. ' +
    'windy-hill-5 는 안내판이라 「포스터·안내판을 사진처럼 쓰지 않는다」에 걸리고, okpo-2 는 진열장 속 상품 디자인이다. ' +
    '나머지 아홉 장은 걸 수는 있지만 **이 글의 주제를 말하지 못한다** — 첨성대도 월정교도 매미성도 「경상도 어디」를 말할 뿐 ' +
    '「경상도 사람이 문장을 어떻게 끝내는가」를 말하지 않는다. 그건 정확히 주제은행이 금지한 「그냥 한국」 사진이다 ' +
    '(제주 Decode 편이 성읍민속마을을 걸 수 있었던 것은 제주 사진이 40장 넘게 있어 고를 여지가 있었기 때문이고, ' +
    '경상 미사용분은 전부 기념물 아니면 해안 풍경이다). ' +
    '요점이 장면이 아니라 **격자**다 — 서술어가 체언이냐 용언이냐, 의문사가 있느냐 없느냐. 축이 둘이면 사진으로 못 찍는다. ' +
    '문패편·화장실편과 같은 판단으로 ChartGyeongsangEnding 을 그렸다. 네 칸의 어미와 예문 다섯은 ' +
    '한국민족문화대백과사전 「동남방언」 의문법어미 항에 적힌 그대로이고, 영어 뜻만 우리가 달아 그 사실을 도표 안에 적었다.',

  'seoul-bus-stop-sign-number-and-colours':
    '재고 332장의 슬러그 72개를 전부 뽑아 훑었다 (2026-09-08). 정류소 기둥·BIT 화면·버스 옆면 계열이 **0장**이다. ' +
    '그중 거리·도로가 찍혀 있을 만한 미사용분 19장(cheonggye-plaza-1~5 · cheonggyecheon-4 · ddp-2·4·6 · ' +
    'hangang-yeouido-1·3 · ikseondong-3·5 · myeongdong-2 · seongsu-underground-2~6)은 **대조표로 만들어 한 장씩 열어 봤다.** ' +
    '결과: cheonggye-plaza-1 은 관광안내판이 선 광장 모퉁이지 정류소가 아니고, 2~5 는 하천·다리·폭포다. ' +
    'ddp 셋은 실내 복도·계단과 외부 계단이며, hangang-yeouido 둘은 공원 안 도로라 버스가 다니지 않는다. ' +
    'ikseondong 둘은 전선이 지나는 골목, seongsu-underground 다섯은 컨테이너 상가(UNDER STAND AVENUE) 간판이다. ' +
    'cheonggyecheon-4 는 기록대로 축제 포스터였고(2025 겨울 청계천의 빛), myeongdong-2 도 기록대로 명동성당이다 — ' +
    '둘 다 금지 항목이다. **열어 보기 전에는 hangang-yeouido-3 의 도로 표지를 정류소로 착각할 뻔했다.** ' +
    '한국관광공사에 정류소 등재 사진이 있는지도 확인이 필요해 data/photo-requests.json 에 걸어 둘 것을 커밋 메시지에 적었다. ' +
    '그리고 이 기사의 요점은 기둥의 생김새가 아니라 **색이 겹친다는 것**이다 — 서울시 자기 표에 Green(초록)이 ' +
    '지선버스와 마을버스 두 줄로 있고 요금이 다르다. 사진 한 장에 다섯 종류가 같이 서지 않으므로 애초에 못 찍는다. ' +
    'PSD편·문패편과 같은 판단으로 ChartSeoulBusColour 를 그렸다. 색·종류·요금은 서울시 「서울 시내버스 체계 및 이용방법」 ' +
    '(버스정책과, 수정일 2026-03-09) 그대로이고, 우리가 만든 것은 요금 축 위의 배치뿐이다. ' +
    '지선이 두 자리에 찍히는 것도 서울시 요금표에 「간지선 1,500원」과 「순환·지선(차등요금) 1,400원」이 따로 있는 그대로다. ' +
    '한글은 도표에 한 자도 넣지 않았다 (샌드박스에 한글 글꼴이 없어 PNG 검수에서 네모로 나온다).',

  'nami-island-from-seoul-ferry-fee-last-train':
    '재고 332장의 슬러그를 전부 뽑았다 (2026-09-08 오후). 남이섬·가평·춘천·북한강 계열이 **0장**이다 — ' +
    '경기도 사진은 suwon-hwaseong(러블리러너 편이 쓰고 있다)과 ilsan-lake-park 둘뿐이다. ' +
    '그래서 「강·숲길·나룻배로 보일 만한」 미사용분 22장(ilsan-lake-park-1~6 · oncheon-suanbo-1~6 · ' +
    'jongmyo-1~6 · geoje-okpo-1~4)을 대조표로 만들어 **한 장씩 눈으로 열었다.** ' +
    '결과: oncheon-suanbo 여섯은 수안보 읍내(징검다리 개울·족욕장·왕관 조형 아치가 선 상가 거리)이고, ' +
    'jongmyo 여섯은 기록대로 정전이 없는 잔디·산책로에 콘크리트 지하도 입구와 파라솔 매점까지 섞여 있다. ' +
    'geoje-okpo 넷은 실내 계단·진열장·전시실이다. ' +
    '▶ **가장 위험했던 것은 ilsan-lake-park-2 다.** 메타세쿼이아가 양쪽으로 늘어선 포장 산책로인데, ' +
    '이것이 남이섬에서 가장 유명한 장면(메타세쿼이아길)과 축소판으로는 구분이 안 된다. ' +
    '열어 보고 이름을 확인했기 때문에 걸렀지 지표로는 안 잡힌다 — 고양시 호수공원 사진을 남이섬 대표사진으로 ' +
    '거는 것은 「다른 장소 사진으로 기사 꾸미기」 그 자체다. 나머지 ilsan 다섯도 연못·정자·라벤더밭이라 무관하다. ' +
    '그리고 이 기사의 요점은 애초에 풍경이 아니라 **시각 셋이 서로 다른 시간에 있다**는 것이다 — ' +
    '서울행 직행버스 16:30, 배 21:00(가을 21:30), 가평역 마지막 서울행 23:11. ' +
    '한 장면에 담기지 않으므로 사진으로 못 찍는다. 버스정류소편·안경편과 같은 판단으로 ' +
    'ChartNamiIslandLastBoat 을 그렸다. 배·버스 시각은 (주)남이섬 공식 페이지, 열차 시각은 한국철도공사 ' +
    '「경춘선 전동열차 평일 시각표(2026.9.1.부터)」 원본 파일 그대로이고, 우리가 만든 것은 시간축 위의 배치뿐이다. ' +
    '한글도 ₩ 기호도 도표에 한 자 넣지 않았다 (오늘 아침 도표에서 ₩ 가 통째로 사라진 사고를 물려받은 규칙이다).',
  ...
  'korea-tax-refund-limits-and-what-to-say':
    '재고 305장을 미사용분 기준으로 훑고, 후보가 될 만한 슬러그는 눈으로 열었다 (2026-09-02). ' +
    '쇼핑·계산대·TAX FREE 간판·출국장 계열에서 **쓸 수 있는 사진이 0장**이다. ' +
    '   myeongdong 5장 중 미사용분은 -2 하나뿐인데, photo-audit 대조표를 열어 보니 ' +
    '   벽돌 고딕 첨탑과 잔디밭 — **명동성당**이다 (올리브영 편에도 같은 판정이 이미 적혀 있고, ' +
    '   이번에 다시 열어서 같은 결론을 확인했다). 세금환급 기사에 성당을 붙이면 「그냥 한국 사진」이다. ' +
    '   namdaemun-market 6장 · gwangjang-market 6장 · incheon-airport 4장은 **전부 이미 다른 기사에 쓰였고** ' +
    '   중복 금지 게이트가 막는다. 미사용분에 남은 것은 궁·산·제주 계열뿐이라 주제와 무관하다. ' +
    '그리고 이 기사의 주인공은 애초에 사진으로 안 보인다 — **영수증 한 장의 금액이 세 경로 중 어느 문을 여는가**다. ' +
    '관광공사 페이지는 15,000 · 1,000,000 · 6,000,000 세 한도를 서로 다른 문단에 흩어 놓았고, ' +
    '한 축에 얹어야 비로소 사다리 모양이 보인다. 그래서 ChartRefundRoutes 를 그렸다 — ' +
    '숫자편·안경편·클리닉편과 같은 판단. ' +
    '도표에는 관광공사가 적은 금액만 넣었고, **5,000,000원(여행 누계)은 축에 올리지 않았다** — ' +
    '영수증 한 장의 한도가 아니라 차원이 다른 숫자라서, 같은 축에 얹는 순간 도표가 거짓말을 한다. 각주로 뺐다.',
  ...
  return {
    ...
    'astro:build:start': ({ logger }) => {
        ...
        for (const file of walk(CONTENT_DIR)) {
          ...
          // ── 4-b. 공공누리 유형이 대장과 맞는가 ──────────────────
          /*
           * try 로 감싸는 이유는 상업 링크 검사와 같다 (아래 665줄 부근의 선례).
           * **검사가 스스로 터져서 빌드를 세우는 일**만은 막는다 —
           * 자동 푸시가 10분마다 돌고, 빌드가 깨지면 그날 발행이 통째로 멈춘다.
           * 2026-08-19 에 그렇게 이틀치를 잃었고, 그때도 원인은 기사가 아니라 도구였다.
           *
           * 검사 자체는 lib/kogl-check.mjs 에서 node 로 시험했다(일부러 깨뜨려 우는 것까지).
           * 그래도 astro 안에서 처음 도는 것은 이번이 처음이므로 안전판을 둔다.
           */
          try {
            const kogl = checkKoglLicences(text, slug, KOGL_REGISTRY);
            fail.push(...kogl.fail);
            warn.push(...kogl.warn);
          } catch (e) {
            warn.push(`${slug}: 공공누리 유형 검사가 스스로 오류를 냈습니다 — 이 기사만 건너뜁니다: ${e.message}`);
          }
