...
const HERO_EXEMPT = {
  ...
  // 내가 틀렸던 지점: 「옷가게 복도라 이태원이 안 보인다」고 적었는데,
  // 다시 열어 보니 itaewon-2 는 **사람 하나가 시장 복도를 혼자 걸어가는** 사진이었다.
  // 이 기사 제목이 「혼자 가도 되는 동네」다. 그림이 그 문장이다.
  // 장소가 어디인지 못 알아보는 것이 문제가 아니라, 무엇을 말하는 사진인가가 기준이어야 했다.
