...
const HERO_EXEMPT = {
  ...
  'biff-2026-tickets-english-page-blank':
    '재고 332장에 부산 사진이 6장 있고, 그 6장이 전부 못 쓰는 것이다 — 오늘 대조표를 만들어 눈으로 다시 열었다. ' +
    'busan-station-1~6 은 라운지·바·루프탑 수영장·긴 공용테이블이 찍힌 **호텔 실내**다 (하운드호텔 부산역점, ' +
    '2026-08-15 부터 기록돼 있던 오매칭). 부산역이 아니고 영화제와는 더더욱 무관하다. ' +
    '영화의전당·센텀시티·해운대 계열은 **0장**이다 (슬러그 전체를 훑었다). ' +
    '남의 사진을 가져올 길도 없다 — 영화제 포스터·스틸·배우 사진은 전부 타인의 저작물이라 ' +
    '스키마의 라이선스 목록(kogl-1·kogl-3·cc0·cc-by·cc-by-sa·own)에 들어올 수 없고, ' +
    '「포스터를 사진처럼 쓰지 않는다」는 금지 항목에도 정면으로 걸린다. ' +
    '게다가 이 기사의 주인공은 건물이 아니라 **예매창이 열리는 시점**이다 — 개막 12일 전이라는 간격은 ' +
    '사진으로 찍을 수 없고 눈금이 있어야 보인다. 안경편·숫자편과 같은 판단으로 ChartBiffBookingWindow 를 그렸다. ' +
    '도표에는 보도자료에 적힌 날짜·시각만 넣었고 D− 값만 우리가 계산했다. 2026 행은 공표된 것이 없어 **비워 두었다.**',

  'seoul-subway-signs-name-on-the-glass':
    '재고 242장을 훑었다 — 지하철 승강장·차내 계열 사진이 0장이다 (2026-08-28 확인). ' +
    '있어도 문제다: 승강장·차내는 출퇴근 승객이 그대로 찍히는 자리라 식별 가능한 사람 문제를 피하기 어렵다. ' +
    '요점 자체가 「대형 표지는 비상문, 소형 표지는 가동문」이라는 배치라서 사진 한 장보다 도식이 정확하다 — ' +
    '클리닉편·문패편과 같은 판단으로 ChartPsdStationName 을 그렸다. 배치는 서울시 보도자료(2023-04-18)의 서술 그대로만 그렸고, ' +
    '서체·배색·영문 병기 여부는 역마다 다를 수 있어 그리지 않았다.',
